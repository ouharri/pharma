...
void templive(char tim[]) // proc�dure permet de structurer la date et le dans dans un string aray pass� dans les parametre .
{
	time_t my_time;
	int ti;
	struct tm *timeinfo;
...
void Logo() // proc�dure qui desine un logo de pharmacie .
{
	templive(box2);
	yellow();
	printf(" %s\n", box2);
...
void welcom() // proc�dure qui fait un intro et un outro de programe  .
{
	system("cls");
	templive(box2);
	yellow();
...
int menu() // proc�dure qui affiche le menu des choix et retourne le choix choisie .
{
	system("cls");
	templive(box2);
	printf(" %s\n", box2);
...
void ajouterProduit(produit *p) // proc�dure qui permet de prendre les information d'un produit .
{
	produit D; //,*p=T,D;
...
void EngistrerProduit(produit p[]) // proc�dure qui permet d'enregistre des produit dans un fichier binaire .
{
	F = fopen("dim.txt", "r");
	if (F != NULL)
		fscanf(F, "%d", &dim);
...
void RestaurerProduit(produit T[]) // proc�dure qui permet de recuperer les produit apartir d'un fichier binaire .
{
	F = fopen("dim.txt", "r");
	produit *p = T, D;
	if (F != NULL)
...
void afficherProduit(produit p, int dim) // proc�dure qui permet d'afficher un seule ou plusieure produit pass� au parametre .
{
	if (dim == 0)
	{
	...
	if (strlen((p.nom)) % 2 == 0)
	{
		for (i = 0; i < round((21 - strlen(p.nom)) / 2 + 0.2); i++)
			printf(" ");
		printf("%s", p.nom);
		for (i = 0; i <= round((21 - strlen(p.nom)) / 2 + 0.2); i++)
			printf(" ");
	...
	if (strlen(box) % 2 == 0)
	{
		for (i = 0; i < round(((18 - strlen(box)) / 2) + 0.2); i++)
			printf(" ");
		printf("%.3f", p.prix);
		for (i = 0; i < round(((18 - strlen(box)) / 2) + 0.2); i++)
			printf(" ");
				...
				if (T[k].code == code)
				{
					...
					else
					{
						...
						if (tesT == 0)
						{
							...
							fprintf(RE, "produit :       Quantit� :     prix unit * Quant :   ");
							fprintf(RE, "\n");
							fflush(stdin);
							fputs(tir, RE);
							fprintf(RE, "\n");
							...
							for (i = 0; i < round(abs((16 - strlen(T[TMP].nom)))); i++)
								fprintf(RE, " ");

							fflush(stdin);
							sprintf(box, "%d", quan);
							fprintf(RE, "%d", quan);
							for (i = 0; i < round(abs((15 - strlen(box)))); i++)
								fprintf(RE, " ");
							...
							for (i = 0; i < round(abs((10 - strlen(box)))); i++)
								fprintf(RE, " ");
							fprintf(RE, "* %d", quan);
...
void triParPrix(produit p[]) // proc�dure qui permet de trier tous les produit selon le prix decroicont .
{
	produit TMP;
	F = fopen("dim.txt", "r");
	fscanf(F, "%d", &dim);
...
void triParAlphabet(produit p[]) // proc�dure qui permet de trier tous les produit selon l'ordre alphabetique decroicont .
{
	produit TMP;
...
void rechercheParCode(produit p[], int code) // proc�dure qui permet de chercher un produit par son code .
{
	F = fopen("dim.txt", "r");
	fscanf(F, "%d", &dim);
	fclose(F);
...
void rechercherParQuantite(produit p[], int quan) // proc�dure qui permet de chercher un produit par sa quantit� .
{
	F = fopen("dim.txt", "r");
	fscanf(F, "%d", &dim);
	fclose(F);
...
void etatDuStock(produit p[]) // proc�dure qui permet d'afficher les produit dont la quantit� est inferieure au 3 .
{
	F = fopen("dim.txt", "r");
	fscanf(F, "%d", &dim);
	fclose(F);
...
void alimenterLeStock(produit p[]) // proc�dure qui permet de faire des modification de quantite dans un produit .
{
	int code, nv;
	test = 0;
...
void suprimerProduit(produit p[], int code) // proc�dure qui permet de chercher un produit par son code et le suprimer .
{
	F = fopen("dim.txt", "r");
	fscanf(F, "%d", &dim);
	fclose(F);
	...
	if (F != NULL)
	{
		do
		{
			...
			if (!strncmp(tim, tim2, 10))
			{
				...
				if (strlen(box) % 2 == 0)
				{
					for (i = 0; i < round(((14 - strlen(box)) / 2) + 0.2); i++)
						printf(" ");
					printf("%05d", quan);
					for (i = 0; i < round(((14 - strlen(box)) / 2) + 0.2); i++)
						printf(" ");
				...
				if (strlen(R.login) % 2 == 0)
				{
					for (i = 0; i < round(((20 - strlen(R.login)) / 2) + 0.2); i++)
						printf(" ");
					printf("%s", R.login);
					for (i = 0; i < round(((20 - strlen(R.login)) / 2) + 0.2); i++)
						printf(" ");
...
void historique() // proc�dure qui permet d'afficher les produit vendu depuit le marche du programe .
{
	produit p;
	test = 0;
	j = 0;
...
float moyenLyoum() // fonction qui permet d'afficher la moyenne des prix des produits vendus en journ�e courante .
{
	char tim2[26];
	produit p;
	float s;
...
float maxLyoum(char boxx[]) // fonction qui permet d'afficher le Max des prix des produits vendus en journ�e courante .
{
	char tim2[26];
	produit p;
	float max = 0;
...
float minLyoum(char boxx[]) // fonction qui permet d'afficher le Min des prix des produits vendus en journ�e courante .
{
	char tim2[26];
	produit p;
